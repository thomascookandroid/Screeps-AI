...
export function getSpeedrunVisualDrawer(state = {}, startTime = Game.time, avgDuration = 100) {
    ...
    return (position = { x: 1, y: 1 }) => {
        ...
        const rooms = Object.values(Game.rooms)
            .map((r) => r.controller)
            .filter((c) => c === null || c === void 0 ? void 0 : c.my);
        const ctrl = rooms[0];
        if (!ctrl || rooms.length > 1)
            return;
        (_a = data.start) !== null && _a !== void 0 ? _a : (data.start = startTime);
        ...
        for (let c = 2; c <= ctrl.level; c++) {
            ...
            visual.text(`RCL ${c}: ${delimit(delay)} (${delimit(data.rcl[c] - data.start)}) - Avg. CP: ${Math.floor((CONTROLLER_LEVELS[c - 1] / delay) * 10) / 10}e/t`, x, y++, { align: "left" });
        }
        visual.text(`Next RCL: ${ctrl.level + 1}`, x, y++, { align: "left", color: "#AACCFF" });
        visual.text(`Progress: ${delimit(ctrl.progress)} / ${delimit(ctrl.progressTotal)} (${Math.floor((ctrl.progress / ctrl.progressTotal) * 1000) / 10}%)`, x, y++, { align: "left" });
        visual.text(`Elapsed: ${delimit(Game.time - data.rcl[ctrl.level])} (${delimit(Game.time - data.start)})`, x, y++, { align: "left" });
        if (data.ept == 0) {
            ...
            visual.text(`Estimated RCL: T+${delimit(tickEstimate - Game.time)} (${delimit(tickEstimate - data.start)})`, x, y++, { align: "left" });
        }
